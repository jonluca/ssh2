...
setup(
  ...
  mustCall((client, server) => {
    ...
    server.on(
      "WRITE",
      mustCall((id, handle, offset, data) => {
        assert(id === 0, `Wrong request id: ${id}`);
        assert.deepStrictEqual(handle, handle_, "handle mismatch");
  ...
  mustCall((client, server) => {
    ...
    client.readdir(
      ...
      mustCall((err, list) => {
        ...
        assert.deepStrictEqual(list, list_.slice(2), "dir list mismatch");
      })
    );
  })
);
...
setup(
  ...
  mustCall((client, server) => {
    ...
    server.on(
      "SYMLINK",
      mustCall((id, linkPath, targetPath) => {
        assert(id === 0, `Wrong request id: ${id}`);
        assert(linkPath === linkPath_, `Wrong link path: ${linkPath}`);
...
setup(
  ...
  mustCall((client, server) => {
    ...
    let buf = [];
    client
      .createReadStream(path_)
      .on(
        "readable",
        mustCallAtLeast(function () {
          let chunk;
          while ((chunk = this.read()) !== null) buf.push(chunk);
        })
      )
      .on(
        "end",
...
setup(
  "ReadStream (fewer bytes than requested)",
  mustCall((client, server) => {
    const path_ = "/foo/bar/baz";
    const handle_ = Buffer.from("hi mom!");
    const data_ = Buffer.from("hello world");
    server
      ...
      .on(
        "READ",
        mustCallAtLeast((id, handle, offset, len) => {
...
setup(
  ...
  mustCall((client, server) => {
    ...
    client.createReadStream(path_).on(
      "error",
      mustCall((err) => {
...
{
  const { client, server } = setup_("SFTP server aborts with exit-status", {
...
function setup(title, cb) {
  ...
  const onSFTP = mustCall(() => {
    if (clientSFTP && serverSFTP) cb(clientSFTP, serverSFTP);
  }, 2);
