...
const Client = require("../dist/lib/client.js").default;
const {
  SSHTTPAgent: HTTPAgent,
  SSHTTPSAgent: HTTPSAgent,
} = require("../dist/lib/http-agents.js");
const Server = require("../dist/lib/server.js").default;
const { KexInit } = require("../dist/lib/protocol/kex.js");
...
{
  const { server } = setup_("Verify host fingerprint (async success)", {
    client: {
      ...clientCfg,
...
{
  const { client, server } = setup_("Verify host fingerprint (sync failure)", {
    client: {
      ...clientCfg,
      hostVerifier: mustCall((key) => {
        return false;
  ...
  client.on("ready", mustNotCall()).on(
    "error",
    mustCall((err) => {
      assert(
        /verification failed/.test(err.message),
      ...
      conn
        ...
        .on(
          ...
          mustCall((err) => {
            console.log(err);
            assert(
              /KEY_EXCHANGE_FAILED/.test(err.message),
    ...
    mustCall((conn) => {
      conn.on(
        ...
        mustCall(() => {
          conn.on(
            "session",
            mustCall((accept, reject) => {
              const session = accept();
              session.on(
                "exec",
                mustCall((accept, reject, info) => {
                  const stream = accept();
                  stream.exit(0);
                  stream.end();
                })
...

{
    ...
    mustCall((conn) => {
      conn.on(
        ...
        mustCall(() => {
          conn.on(
            "request",
            mustCall((accept, reject, name, info) => {
  ...
  client
    ...
    .on(
      "tcp connection",
      mustCall((details, accept, reject) => {
        assert.deepStrictEqual(
          details,
          {
            destIP: "good",
...

{
  ...
  const { client, server } = setup_("Server greeting", {
    client: {
      ...clientCfg,
      ident: "node.js rules",
    },
...
{
  const { client, server } = setup_("Correct ident parsing", {
    client: {
      ...clientCfg,
      ident: "node.js rules\n",
    },
...

{
  ...
  const { client, server } = setup_("Server banner", {
    client: clientCfg,
    server: {
      ...serverCfg,
      banner: BANNER,
  ...
  client.on("greeting", mustNotCall()).on(
    "banner",
    mustCall((message) => {
            ...
            mustCall((accept, reject) => {
              ...
              session.on(
                ...
                mustCall((accept, reject, info) => {
                  assert(
                    info.name === "netconf",
                    `Wrong subsystem name: ${info.name}`
                  );
...
{
  const { client, server } = setup_("Handshake errors are emitted", {
    client: {
      ...clientCfg,
    ...
    mustCall((conn) => {
      ...
      conn
        .on(
          ...
          mustCall((err) => {
            assert(
              /signature generation failed/i.test(err.message),
              "Wrong error message"
            );
          })
        )
...
{
  const { client, server } = setup_("Rekeying with AES-GCM", {
    client: {
      ...clientCfg,
    ...
    .on(
      ...
      mustCall((info) => {
        switch (++handshakes) {
          ...
          case 2:
            assert(
              info.cs.compress === "zlib@openssh.com",
              "wrong compress value"
            );
            assert(
              info.sc.compress === "zlib@openssh.com",
              "wrong compress value"
            );
            break;
        }
...
{
  const { client, server } = setup_("Large data compression", {
    client: {
      ...clientCfg,
      algorithms: { compress: ["zlib"] },
    },
            ...
            conn.on(
              ...
              mustCall((accept, reject) => {
                accept().on(
                  ...
                  mustCall((accept, reject, info) => {
                    const stream = accept();
                    for (let i = 0; i < chunkCount; ++i) stream.write(chunk);
                    stream.exit(0);
                    stream.end();
                  })
                );
...
{
  const { client, server } = setup_("Debug output", {
    client: {
      ...clientCfg,
...
{
  const { server } = setup_("HTTP agent", {
        ...
        .on(
          ...
          mustCall(() => {
            conn.on(
              ...
              mustCall((accept, reject, info) => {
                assert(
                  info.destIP === "localhost",
                  `Wrong destIP: ${info.destIP}`
                );
                assert(
                  info.destPort === httpServer.address().port,
                  `Wrong destPort: ${info.destPort}`
                );
                assert(
                  info.srcIP === "localhost",
                  `Wrong srcIP: ${info.srcIP}`
                );
...
{
  const { server } = setup_("HTTPS agent", {
    // No automatic client, the agent will create one
    ...
    httpsServer.listen(0, "localhost", () => {
      ...
      https
        ...
        .on("error", (err) => {
          ...
          if (!/write after end/i.test(err.message)) throw err;
        });
    });
  });
          ...
          mustCall(() => {
            conn.on(
              ...
              mustCall((accept, reject, info) => {
                ...
                assert(
                  info.destPort === httpsServer.address().port,
                  `Wrong destPort: ${info.destPort}`
                );
                assert(
                  info.srcIP === "localhost",
...
[
  ...
  const { client, server } = setup_(`Client algorithms option (${info.desc})`, {
    client: {
      ...clientCfg,
...
{
  const { client } = setup_(`Safely end() from Client 'error' event handler`, {
    client: clientCfg,
    noClientError: true,
    noForceClientReady: true,
  });
...
{
  const { client } = setup_("Only one client error on connection failure", {
    client: clientCfg,
    noClientError: true,
    noForceClientReady: true,
  });
