...
for (const file of readdirSync(FIXTURES_DIR, { withFileTypes: true })) {
  if (file.isFile()) chmodSync(join(FIXTURES_DIR, file.name), 0o600);
}

{
  // Get OpenSSH client version first
  const { error, stderr, stdout } = spawnSync(opensshPath, ["-V"], SPAWN_OPTS);
...
[
  { desc: "RSA user key (old OpenSSH)", clientKey: fixtureKey("id_rsa") },
  {
    desc: "RSA user key (new OpenSSH)",
    clientKey: fixtureKey("openssh_new_rsa"),
  },
  { desc: "DSA user key", clientKey: fixtureKey("id_dsa") },
  { desc: "ECDSA user key", clientKey: fixtureKey("id_ecdsa") },
].forEach((test) => {
  const { desc, clientKey } = test;
  const username = "KeyUser";
  const { server } = setup(desc, {
    client: {
      username,
      ...
      conn
        .on(
          ...
          mustCallAtLeast((ctx) => {
            ...
            if (ctx.signature) {
              assert(
                clientKey.key.verify(ctx.blob, ctx.signature) === true,
...
[
  { desc: "RSA host key (old OpenSSH)", hostKey: fixture("id_rsa") },
  { desc: "RSA host key (new OpenSSH)", hostKey: fixture("openssh_new_rsa") },
  { desc: "DSA host key", hostKey: fixture("ssh_host_dsa_key") },
  { desc: "ECDSA host key", hostKey: fixture("ssh_host_ecdsa_key") },
  { desc: "PPK", hostKey: fixture("id_rsa.ppk") },
].forEach((test) => {
...
// Various edge cases
{
  const clientKey = fixtureKey("openssh_new_rsa");
  const username = "KeyUser";
  const { server } = setup("Server closes stdin too early", {
    client: {
      username,
      privateKeyPath: clientKey.fullPath,
    },
...
function setup(title, configs) {
  ...
  const timeout = typeof timeout_ === "number" ? timeout_ : CLIENT_TIMEOUT;
  ...
  server
    ...
    .on(
      "connection",
      mustCall((conn) => {
        conn.on("error", onError).on("ready", serverReadyFn);
        server.close();
      })
    )
