...
import { isParsedKey, parseKey } from "./protocol/keyParser";
import {
  makeBufferParser,
  readUInt32BE,
  writeUInt32BE,
...
export function createAgent(path) {
...
export const AgentProtocol: any = (() => {
  ...
  class _AgentProtocol extends Duplex {
    /*
        Notes:
    ...
    */
              ...
              */
              ...
              if (pubKey.type === "ssh-rsa") {
                if (flagsVal & SSH_AGENT_RSA_SHA2_256) {
                  ctx = "rsa-sha2-256";
                  // @ts-expect-error TS(2322): Type '"sha256"' is not assignable to type 'undefin... Remove this comment to see the full error message
                  flags.hash = "sha256";
                } else if (flagsVal & SSH_AGENT_RSA_SHA2_512) {
                  ctx = "rsa-sha2-512";
                  // @ts-expect-error TS(2322): Type '"sha512"' is not assignable to type 'undefin... Remove this comment to see the full error message
                  flags.hash = "sha512";
                }
              }
    ...
    getIdentitiesReply(req, keys) {
      ...
      */
      ...
      for (let i = 0; i < newKeys.length; ++i) {
        ...
        writeUInt32BE(buf, (pubKey as any).length, p);
        (pubKey as any).copy(buf, (p += 4));

        writeUInt32BE(
          buf,
          (comment as any).length,
          (p += (pubKey as any).length)
        );
        p += 4;
        if ((comment as any).length) {
          (comment as any).copy(buf, p);
          p += (comment as any).length;
        }
      }
    ...
    signReply(req, signature) {
      ...
      */
      ...
      (buf as any).utf8Write(sigFormat, (p += 4), sigFormatLen);
