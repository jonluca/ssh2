...
import { init as cryptoInit } from "./protocol/crypto";

import { KexInit } from "./protocol/kex";
...
import { writeUInt32BE } from "./protocol/utils";
import {
  Channel,
  MAX_WINDOW,
  PACKET_SIZE,
...
class PKAuthContext extends AuthContext {
  ...
  constructor(
    ...
    service,
    method,
    { keyAlgo, key, signature, blob },
    cb
  ) {
    super(protocol, username, service, method, cb);

    this.key = { algo: keyAlgo, data: key };
...
class Session extends EventEmitter {
  ...
  subtype: any;
  type: any;
  constructor(client, { sender, window, packetSize }, localChan) {
    super();
...
class Server extends EventEmitter {
  ...
  constructor(cfg, listener) {
    ...
    // @ts-expect-error TS(2322): Type '{ cipher: any; mac: any; compress: any; lang... Remove this comment to see the full error message
...
class Client extends EventEmitter {
  ...
  constructor(
    ...
    debug,
    server,
    { keepaliveInterval, keepaliveCountMax, greeting, banner }
  ) {
    super();
    ...
    const kaIntvl =
      typeof keepaliveInterval === "number" &&
      isFinite(keepaliveInterval) &&
      keepaliveInterval > 0
        ? keepaliveInterval
        ...
        : -1;
    const kaCountMax =
      typeof keepaliveCountMax === "number" &&
      isFinite(keepaliveCountMax) &&
      keepaliveCountMax >= 0
        ? keepaliveCountMax
    ...
    const proto = (this._protocol = new Protocol({
      ...
      messageHandlers: {
        ...
        CHANNEL_OPEN: (p, info) => {
          ...
          // allocation
          if (
            (info.type === "session" && this.noMoreSessions) ||
            !this.authenticated
          ) {
          ...
          const reserveChannel = () => {
            ...
            if (localChan === -1) {
              ...
              if (debug) {
                debug(
                  "Automatic rejection of incoming channel open: " +
          ...
          switch (info.type) {
            ...
            case "direct-streamlocal@openssh.com":
              if (
                listenerCount(this, "openssh.streamlocal") &&
                reserveChannel()
              ) {
                accept = () => {
                  if (replied) return;
            ...
            default:
              ...
              if (debug) {
                debug(
                  `Automatic rejection of unsupported incoming channel open${` type: ${info.type}`}`
                );
              }
          }
          ...
          if (reason === undefined) {
            ...
            if (debug) {
              debug(
                `Automatic rejection of unexpected incoming channel open${` for: ${info.type}`}`
              );
            }
          }
        ...
        GLOBAL_REQUEST: (p, name, wantReply, data) => {
          ...
          // @ts-expect-error TS(2345): Argument of type '{ type: null; buf: null; }' is n... Remove this comment to see the full error message
    ...
    function sendReplies() {
      while (
        ...
        (unsentGlobalRequestsReplies[0] as any).type
      ) {
...
function openChannel({ _chanMgr, _protocol }, type, opts, cb) {
  ...
  switch (type) {
    case "forwarded-tcpip":
      _protocol.forwardedTcpip(localChan, initWindow, maxPacket, opts);
      break;
    case "x11":
      _protocol.x11(localChan, initWindow, maxPacket, opts);
      break;
...
export { Client as IncomingClient };
