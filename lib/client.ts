...
import { createHash, getHashes, randomFillSync } from "crypto";
...
import { init as cryptoInit } from "./protocol/crypto.js";
import Protocol from "./protocol/Protocol.js";
import { parseKey } from "./protocol/keyParser.js";
import { SFTP } from "./protocol/SFTP.js";
...
import { AgentContext, createAgent, isAgent } from "./agent.js";
import {
  Channel,
  MAX_WINDOW,
  PACKET_SIZE,
...
export class Client extends EventEmitter {
  ...
  connect(cfg) {
    ...
    this.config.keepaliveCountMax =
      typeof cfg.keepaliveCountMax === "number" && cfg.keepaliveCountMax >= 0
    ...
    this.config.readyTimeout =
      typeof cfg.readyTimeout === "number" && cfg.readyTimeout >= 0
        ...
        : 20000;
    this.config.ident =
      typeof cfg.ident === "string" || Buffer.isBuffer(cfg.ident)
        ? cfg.ident
        : undefined;
    ...
    if (typeof cfg.algorithms === "object" && cfg.algorithms !== null) {
      ...
      // @ts-expect-error TS(2322): Type '{ cipher: undefined; mac: undefined; compres... Remove this comment to see the full error message
    ...
    this.config.localHostname =
      typeof cfg.localHostname === "string" ? cfg.localHostname : undefined;
    this.config.localUsername =
      typeof cfg.localUsername === "string" ? cfg.localUsername : undefined;
    ...
    let authHandler = (this.config.authHandler =
      typeof cfg.authHandler === "function" || Array.isArray(cfg.authHandler)
        ? cfg.authHandler
        : undefined);
    ...
    const debug = (this.config.debug =
      typeof cfg.debug === "function" ? cfg.debug : undefined);
    ...
    if (typeof cfg.hostVerifier === "function") {
      ...
      let hasher;
      if (HASHES.includes(cfg.hostHash)) {
    ...
    const proto = (this._protocol = new Protocol({
      ...
      onHeader: ({ versions, greeting }) => {
        sawHeader = true;
        this._remoteVer = versions.software;
        if (greeting) this.emit("greeting", greeting);
      },
      ...
      messageHandlers: {
          ...
          if (curAuth.type === "agent") {
            ...
            proto.authPK(curAuth.username, curAuth.key, (buf, cb) => {
              ...
              if (signature instanceof Error) {
                ...
                (signature as any).level = "client-authentication";
        ...
        USERAUTH_INFO_REQUEST: (p, name, instructions, prompts) => {
          if (curAuth.type === "keyboard-interactive") {
            ...
            if (nprompts === 0) {
              debug &&
                debug("Client: Sending automatic USERAUTH_INFO_RESPONSE");
        ...
        CHANNEL_OPEN_CONFIRMATION: (
          p,
          { recipient, sender, window, packetSize }
        ) => {
    ...
    const sendKA = () => {
      if (++kacount > kacountmax) {
        ...
        if (sock.readable) {
          ...
          (err as any).level = "client-timeout";
          this.emit("error", err);
          sock.destroy();
        }
        return;
    ...
    const doNextAuth = (nextAuth) => {
      ...
      if (nextAuth === false) {
        ...
        (err as any).level = "client-authentication";
        this.emit("error", err);
        this.end();
        return;
      }
  ...
  exec(cmd, opts, cb) {
    ...
    openChannel(this, "session", extraOpts, (err, chan) => {
      ...
      if (
        this.config.allowAgentFwd === true ||
        (opts && opts.agentForward === true && this._agent !== undefined)
      ) {
        // @ts-expect-error TS(2345): Argument of type '() => void' is not assignable to... Remove this comment to see the full error message
...
function openChannel({ _chanMgr, _protocol }, type, opts, cb) {
  ...
  const localChan = _chanMgr.add(wrapper);
  ...
  switch (type) {
    case "session":
    case "sftp":
      _protocol.session(localChan, initWindow, maxPacket);
      break;
    case "direct-tcpip":
      _protocol.directTcpip(localChan, initWindow, maxPacket, opts);
      break;
    case "direct-streamlocal@openssh.com":
      _protocol.openssh_directStreamLocal(
        localChan,
        initWindow,
        maxPacket,
        opts
...
function reqPty({ outgoing, _callbacks, _client }, opts, cb) {
  let rows = 24;
  let cols = 80;
  let width = 640;
  let height = 480;
...
function reqAgentFwd({ outgoing, _client, _callbacks }, cb) {
  const wantReply = typeof cb === "function";
  ...
  if (_client._agentFwdEnabled) {
    wantReply && cb(false);
    return;
  }

  _client._agentFwdEnabled = true;

  _callbacks.push((had_err) => {
    if (had_err) {
      _client._agentFwdEnabled = false;
      if (wantReply) {
        cb(
          had_err !== true
            ? had_err
  ...
  _client._protocol.openssh_agentForward(outgoing.id, true);
}
...
function reqExec(chan, cmd, { allowHalfOpen }, cb) {
  if (chan.outgoing.state !== "open") {
    cb(new Error("Channel is not open"));
    return;
  }
  ...
  chan._callbacks.push((had_err) => {
    ...
    chan.subtype = "exec";
    chan.allowHalfOpen = allowHalfOpen !== false;
    cb(undefined, chan);
  });
...
function reqEnv({ outgoing, _client }, env) {
  ...
  keys.forEach((key) => {
    const val = env[key];
    _client._protocol.env(outgoing.id, key, val, false);
  });
}
...
function onCHANNEL_OPEN(self, info) {
  ...
  const accept = () => {
    ...
    self._protocol.channelOpenConfirm(
      info.sender,
      localChan,
      MAX_WINDOW,
      PACKET_SIZE
  ...
  switch (info.type) {
    ...
    case "forwarded-streamlocal@openssh.com":
      if (
        self._forwardingUnix[data.socketPath] !== undefined &&
        reserveChannel()
      ) {
...
const randomCookie = (() => {
  ...
  return () => {
    ...
    return (buffer as any).hexSlice(0, 16);
  };
})();
  ...
  if (
    ...
    client._callbacks.push((had_err, data) => {
      ...
      while (bufferParser.avail()) {
        ...
        const keyPublic = (key as any).getPublicSSH();
        ...
        let algo;
        if (type !== (key as any).type) {
          if ((key as any).type === "ssh-rsa") {
            switch (type) {
              case "rsa-sha2-256":
                algo = "sha256";
                break;
        ...
        if ((key as any).verify(verifyData, value, algo) === true)
          ret.push(key);
      }
      sigParser.clear();
      bufferParser.clear();
