...
import { inherits, isDate } from "util";
...
export class SFTP extends EventEmitter {
  ...
  subtype: any;
  type: any;
  constructor(client, { type, incoming, outgoing }, cfg) {
    super();
  ...
  open(path, flags_, attrs, cb) {
    ...
    */
    ...
    (buf as any).utf8Write(path, (p += 4), pathLen);
      ...
      if (openErr) {
        ...
        if (position === null) {
          const tryStat = (er, { size }) => {
            ...
            writeAll(this, handle, buffer, 0, buffer.length, size, callback);
          };
          this.fstat(handle, tryStat);
          return;
        }
  ...
  unlink(filename, cb) {
    ...
    */
    ...
    (buf as any).utf8Write(filename, (p += 4), fnameLen);
  ...
  rename(oldPath, newPath, cb) {
    ...
    */
    ...
    (buf as any).utf8Write(oldPath, (p += 4), oldLen);
    writeUInt32BE(buf, newLen, (p += oldLen));
    (buf as any).utf8Write(newPath, (p += 4), newLen);
  ...
  symlink(targetPath, linkPath, cb) {
    ...
    */
    ...
    if (this._isOpenSSH) {
      ...
      (buf as any).utf8Write(targetPath, (p += 4), targetLen);
      writeUInt32BE(buf, linkLen, (p += targetLen));
      (buf as any).utf8Write(linkPath, (p += 4), linkLen);
    } else {
  ...
  ext_openssh_rename(oldPath, newPath, cb) {
    ...
    */
    ...
    (buf as any).utf8Write("posix-rename@openssh.com", (p += 4), 24);
  ...
  ext_openssh_statvfs(path, cb) {
    ...
    */
    ...
    (buf as any).utf8Write("statvfs@openssh.com", (p += 4), 19);
  ...
  ext_openssh_fstatvfs(handle, cb) {
    ...
    */
    ...
    (buf as any).utf8Write("fstatvfs@openssh.com", (p += 4), 20);
  ...
  ext_openssh_hardlink(oldPath, newPath, cb) {
    ...
    */
    ...
    writeUInt32BE(buf, 20, p);
    (buf as any).utf8Write("hardlink@openssh.com", (p += 4), 20);
  ...
  ext_openssh_fsync(handle, cb) {
    ...
    */
    ...
    (buf as any).utf8Write("fsync@openssh.com", (p += 4), 17);
  ...
  ext_openssh_lsetstat(path, attrs, cb) {
    ...
    */
    ...
    writeUInt32BE(buf, 20, p);
    (buf as any).utf8Write("lsetstat@openssh.com", (p += 4), 20);
  ...
  ext_openssh_expandPath(path, cb) {
    ...
    */
    ...
    (buf as any).utf8Write("expand-path@openssh.com", (p += 4), 23);
  ...
  ext_copy_data(srcHandle, srcOffset, len, dstHandle, dstOffset, cb) {
    ...
    */
    ...
    p += 4;
    (buf as any).utf8Write("copy-data", p, 9);
    p += 9;
  ...
  status(reqid, code, message) {
    ...
    p += 4;
    if (msgLen) {
      (buf as any).utf8Write(message, p, msgLen);
      p += msgLen;
    }
  ...
  data(reqid, data, encoding) {
    ...
    if (dataLen) {
      ...
      else if (isUTF8) (buf as any).utf8Write(data, (p += 4), dataLen);
  ...
  name(reqid, names) {
    ...
    for (let i = 0; i < count; ++i) {
...

      {
        ...
        p += 4;
        if (len) {
          (buf as any).utf8Write(filename, p, len);
          p += len;
        }
      }

      {
        ...
        p += 4;
        if (len) {
          (buf as any).utf8Write(longname, p, len);
          p += len;
        }
      }
      ...
      const attr = attrs[i];
      if (attr) {
        writeUInt32BE(buf, (attr as any).flags, p);
        p += 4;
        if ((attr as any).flags && (attr as any).bytes) {
          buf.set((attr as any).bytes, p);
          p += (attr as any).nb;
        }
      } else {
...
export function stringToFlags(str) {
...
export const flagsToString = (() => {
  ...
  return (flags) => {
    for (const key of stringFlagMapKeys) {
...
function doFatalSFTPError(sftp, msg, noDebug) {
  ...
  (err as any).level = "sftp-protocol";
...
function requestLimits(sftp, cb) {
  ...
  */
  ...
  (buf as any).utf8Write("limits@openssh.com", (p += 4), 18);
    ...
    */
    ...
    if (extensions["limits@openssh.com"] === "1") {
      return requestLimits(
        sftp,
        (err, { maxPktLen, maxReadLen, maxWriteLen, maxOpenHandles }) => {
          if (!err) {
            if (maxPktLen > 0) sftp._maxOutPktLen = maxPktLen;
            if (maxReadLen > 0) sftp._maxReadLen = maxReadLen;
            if (maxWriteLen > 0) sftp._maxWriteLen = maxWriteLen;
            sftp.maxOpenHandles =
              maxOpenHandles > 0 ? maxOpenHandles : Infinity;
          }
          sftp.emit("ready");
        }
      );
    ...
    */
    ...
    if (count !== undefined) {
      ...
      for (let i = 0; i < count; ++i) {
        ...
        if (attrs === undefined) {
          // @ts-expect-error TS(2322): Type 'undefined' is not assignable to type 'never[... Remove this comment to see the full error message
          names = undefined;
          break;
        }
        // @ts-expect-error TS(2322): Type 'any' is not assignable to type 'never'.
...
class WriteStream {
  ...
  _write(data, encoding, cb) {
    ...
    if (!Buffer.isBuffer(this.handle)) {
      return this.once("open", function (this: any) {
...
// @ts-expect-error TS(2551): Property '_destroy' does not exist on type 'WriteS... Remove this comment to see the full error message
WriteStream.prototype._destroy = ReadStream.prototype._destroy;

// There is no shutdown() for files.
(WriteStream.prototype as any).destroySoon = WriteStream.prototype.end;
