...
export const CIPHER_INFO = {
  "chacha20-poly1305@openssh.com": info(
    "chacha20",
    8,
    64,
    0,
...
class ChaChaPolyCipherNative {
  ...
  encrypt(packet) {
    ...
    const polyKey = createCipheriv(
      "chacha20",
      this._encKeyMain,
      POLY1305_OUT_COMPUTE
    ).update(POLY1305_ZEROS);

    // Encrypt packet length
    const pktLenEnc = createCipheriv(
      "chacha20",
      this._encKeyPktLen,
      POLY1305_OUT_COMPUTE
    ...
    const payloadEnc = createCipheriv(
      "chacha20",
      this._encKeyMain,
      POLY1305_OUT_COMPUTE
  ...
  encrypt(packet) {
    ...
    if (this._macETM) {
      ...
      const encrypted = this._cipherInstance.update(
        new Uint8Array(packet.buffer, packet.byteOffset + 4, packet.length - 4)
      );
  ...
  decrypt(data, p, dataLen) {
    ...
    while (p < dataLen) {
      ...
      if (this._lenPos < 4) {
        ...
        const decLenBytes = createDecipheriv(
          "chacha20",
          this._decKeyPktLen,
          POLY1305_OUT_COMPUTE
        ).update(this._lenBuf);
      ...
      if (this._blockPos < this._block.length) {
        ...
        if (this._macETM) {
          ...
          this._macInstance.update(
            new Uint8Array(decrypted.buffer, decrypted.byteOffset, 4)
          );
...
export function createCipher(config) {
  ...
  if (
    !Buffer.isBuffer(outbound.cipherKey) ||
  ...
  switch (outbound.cipherInfo.sslName) {
    ...
    default: {
      ...
      if (
        !Buffer.isBuffer(outbound.macKey) ||
...
export function createDecipher(config) {
  if (typeof config !== "object" || config === null)
    throw new Error("Invalid config");
  ...
  if (
    typeof inbound.decipherInfo !== "object" ||
  ...
  if (
    !Buffer.isBuffer(inbound.decipherKey) ||
  ...
  switch (inbound.decipherInfo.sslName) {
    ...
    default: {
      ...
      if (
        !Buffer.isBuffer(inbound.macKey) ||
...
export const bindingAvailable = !!binding;
export default {
  CIPHER_INFO,
  MAC_INFO,
  bindingAvailable,
