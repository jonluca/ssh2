...
import { CIPHER_INFO } from "./crypto";
import { eddsaSupported, SUPPORTED_CIPHER } from "./constants";
import {
  bufferSlice,
  makeBufferParser,
  readString,
...
function genOpenSSHRSAPub(n, e) {
  ...
  (publicKey as any).utf8Write("ssh-rsa", 4, 7);
...
function genOpenSSHDSAPub(p, q, g, y) {
  ...
  writeUInt32BE(publicKey, 7, 0);
  (publicKey as any).utf8Write("ssh-dss", 4, 7);
...
function genOpenSSHEdPub(pub) {
  ...
  (publicKey as any).utf8Write("ssh-ed25519", 4, 11);
...
function genOpenSSHECDSAPub(oid, Q) {
  ...
  (publicKey as any).utf8Write(`ecdsa-sha2-${curveName}`, 4, 19);

  writeUInt32BE(publicKey, 8, 23);
  (publicKey as any).utf8Write(curveName, 27, 8);
...
const BaseKey = {
  sign: (() => {
    if (typeof sign_ === "function") {
      return function sign(this: any, data, algo) {
  ...
  verify: (() => {
    if (typeof verify_ === "function") {
      return function verify(this: any, data, signature, algo) {
...
OpenSSH_Private.prototype = BaseKey;
{
  ...
  OpenSSH_Private.parse = (str, passphrase) => {
    ...
    const magic = (data as any).utf8Slice(0, 15);
    ...
    const kdfName = readString(data, (data as any)._pos, true);
    ...
    const kdfOptions = readString(data, (data as any)._pos);
    ...
    if ((data as any)._pos + 3 >= data.length)
      return new Error("Malformed OpenSSH private key");
    const keyCount = readUInt32BE(data, (data as any)._pos);
    (data as any)._pos += 4;
    ...
    if (keyCount > 0) {
      ...
      for (let i = 0; i < keyCount; ++i) {
        const pubData = readString(data, (data as any)._pos);
      ...
      let privBlob = readString(data, (data as any)._pos);
      ...
      if (cipherKey !== undefined) {
        ...
        try {
          ...
          if (encInfo.authLen > 0) {
            if (data.length - (data as any)._pos < encInfo.authLen)
              return new Error("Malformed OpenSSH private key");
            decipher.setAuthTag(
              bufferSlice(
                data,
                (data as any)._pos,
                ((data as any)._pos += encInfo.authLen)
              )
            );
          }
      ...
      if ((data as any)._pos !== data.length)
        return new Error("Malformed OpenSSH private key");
  ...
  // @ts-expect-error TS(1252): Function declarations are not allowed inside block... Remove this comment to see the full error message
  ...
  OpenSSH_Old_Private.parse = (str, passphrase) => {
    ...
    if (headers !== undefined) {
      ...
      for (const header of headers) {
        ...
        if (header.slice(0, sepIdx) === "DEK-Info") {
          ...
          if (!supportedOpenSSLCiphers.includes(cipherName)) {
            return new Error(
    ...
    switch (m[1]) {
      case "RSA":
        ...
        try {
          reader = new (Ber as any).Reader(privBlob);
          reader.readSequence();
...
PPK_Private.prototype = BaseKey;
{
  ...
  PPK_Private.parse = (str, passphrase) => {
    ...
    (macData as any).utf8Write(type, (p += 4), typeLen);
    writeUInt32BE(macData, cipherNameLen, (p += typeLen));
    (macData as any).utf8Write(cipherName, (p += 4), cipherNameLen);
    writeUInt32BE(macData, commentLen, (p += cipherNameLen));
    (macData as any).utf8Write(comment, (p += 4), commentLen);
    ...
    let pubSSH;
    let privPEM;
    (pubBlob as any)._pos = 0;
    skipFields(pubBlob, 1); // skip (duplicate) key type
    switch (type) {
      case "ssh-rsa": {
        const e = readString(pubBlob, (pubBlob as any)._pos);
        if (e === undefined) return new Error("Malformed PPK public key");
        const n = readString(pubBlob, (pubBlob as any)._pos);
        ...
        const p = readString(privBlob, (privBlob as any)._pos);
        if (p === undefined) return new Error("Malformed PPK private key");
        const q = readString(privBlob, (privBlob as any)._pos);
        if (q === undefined) return new Error("Malformed PPK private key");
        const iqmp = readString(privBlob, (privBlob as any)._pos);
      ...
      case "ssh-dss": {
        const p = readString(pubBlob, (pubBlob as any)._pos);
        if (p === undefined) return new Error("Malformed PPK public key");
        const q = readString(pubBlob, (pubBlob as any)._pos);
        if (q === undefined) return new Error("Malformed PPK public key");
        const g = readString(pubBlob, (pubBlob as any)._pos);
        if (g === undefined) return new Error("Malformed PPK public key");
        const y = readString(pubBlob, (pubBlob as any)._pos);
...
function OpenSSH_Public(this: any, type, comment, pubPEM, pubSSH, algo) {
  this.type = type;
  this.comment = comment;
...
OpenSSH_Public.prototype = BaseKey;
{
  ...
  OpenSSH_Public.parse = (str) => {
    ...
    const type = readString(data, (data as any)._pos, true);
...
function RFC4716_Public(this: any, type, comment, pubPEM, pubSSH, algo) {
  this.type = type;
  this.comment = comment;
  this[SYM_PRIV_PEM] = null;
  this[SYM_PUB_PEM] = pubPEM;
...
RFC4716_Public.prototype = BaseKey;
{
  ...
  RFC4716_Public.parse = (str) => {
    ...
    // @ts-expect-error TS(2322): Type 'Buffer' is not assignable to type 'string'.
    ...
    switch (type) {
      case "ssh-rsa": {
        const e = readString(data, (data as any)._pos);
        if (e === undefined) return new Error("Malformed RFC4716 public key");
        const n = readString(data, (data as any)._pos);
        ...
        pubPEM = genOpenSSLRSAPub(n, e);
        // @ts-expect-error TS(2322): Type 'Buffer' is not assignable to type 'null'.
        pubSSH = genOpenSSHRSAPub(n, e);
        break;
      }
      case "ssh-dss": {
        const p = readString(data, (data as any)._pos);
        if (p === undefined) return new Error("Malformed RFC4716 public key");
        const q = readString(data, (data as any)._pos);
        if (q === undefined) return new Error("Malformed RFC4716 public key");
        const g = readString(data, (data as any)._pos);
        if (g === undefined) return new Error("Malformed RFC4716 public key");
        const y = readString(data, (data as any)._pos);
...
function parseDER(data, baseType, comment, fullType) {
  ...
  switch (baseType) {
    ...
    case "ecdsa-sha2-nistp521": {
      ...
      // @ts-expect-error TS(2322): Type 'string' is not assignable to type 'null'.
      pubPEM = genOpenSSLECDSAPub(oid, ecpub);
      // @ts-expect-error TS(2322): Type 'Buffer | undefined' is not assignable to typ... Remove this comment to see the full error message
      pubSSH = genOpenSSHECDSAPub(oid, ecpub);
      break;
    }
    default:
...
export function isParsedKey(val) {
  if (!val) return false;
  return typeof val[SYM_DECRYPTED] === "boolean";
}

export function parseKey(data?: Buffer | string, passphrase?: string | Buffer) {
  if (isParsedKey(data)) return data;

  let origBuffer;
  if (Buffer.isBuffer(data)) {
    origBuffer = data;
    data = (data as any).utf8Slice(0, data.length).trim();
...
export const parseDERKey = (data, type) => parseDER(data, type, "", type);
export default {
  isParsedKey,
  isSupportedKeyType,
  parseDERKey,
